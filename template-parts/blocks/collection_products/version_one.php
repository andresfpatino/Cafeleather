...
if( have_rows('group_6_products') ):
    while( have_rows('group_6_products') ): the_row(); 
        ...
        <?php endif; 
        $id_secondary_products = get_sub_field('secondary_products'); 
        if( $id_secondary_products ): ?>                     
            <div class="grid-products --secondary-products"> <?php
                        
                $arg = array(
                    'post_type' => 'product',
                    'post__in' => $id_secondary_products,
                    'posts_per_page' => 4,
                    'order' => 'DESC',
                    'orderby' => 'NAME'
                );  $secondaryProducts = new WP_Query($arg);
            
                if($secondaryProducts->have_posts()) :
                    while($secondaryProducts->have_posts()) : $secondaryProducts->the_post() ; ?>                    
                        <div class="product"> 
                            <a class="product-link" href="<?php the_permalink(); ?>">
                                <div class="imagebox"> <?php                          
                                    global $product;                                                                       
